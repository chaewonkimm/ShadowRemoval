...
class PyramidPooling(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = torch.cat([F.interpolate(stage(feats), size=(h, w), mode='nearest') for stage in self.stages] + [feats], dim=1)
        return self.relu(self.bottleneck(priors))

class MaskedAutoencoderViT(nn.Module):
    """Masked Autoencoder with VisionTransformer backbone with global frequency additional token"""
    ...
    def unpatchify(self, x, H, W):
        ...
        w = W // p
        expected_tokens = h * w
        if x.shape[1] == expected_tokens + 1:
            x = x[:, :-1, :]
        assert x.shape[1] == expected_tokens, f"Expected {expected_tokens} tokens but got {x.shape[1]}"
    ...
    def forward_encoder(self, imgs):
        ...
        freq_token = self.freq_fc(freq_vector)
        freq_token = freq_token.unsqueeze(1)
        x = torch.cat([x, freq_token], dim=1)

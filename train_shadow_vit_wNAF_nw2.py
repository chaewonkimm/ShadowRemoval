...
def get_matte_weight_map(matte, device):
    matte = matte.to(device)
    weight_map = (matte - matte.min()) / (matte.max() - matte.min() + 1e-8)
    return weight_map

def matte_weighted_charbonnier_loss(output, target, matte, epsilon=1e-3, weight_coef=1.0):
    device = output.device
    weight_map = get_matte_weight_map(matte, device)
...
parser.add_argument('--vit_img_size', type=int, default=256)
...
parser.add_argument('--experiment_name', type=str, default="finetune_vit_nafnet_matteloss")
    ...
    with torch.no_grad():
        for data, label, fname in val_loader:
            ...
            for crop in crops:
                ...
                # matte patch merge
                matte_crop = merge_image_overlap(matte_patches, positions, crop_size=256, resolution=crop.shape, overlap_size=args.overlap_size, blend_mode='gaussian')
                matte_crops.append(matte_crop)
            outputs = merge_sliding_crops(processed_crops, crop_positions, original_width=W, overlap_size=args.overlap_size)
            matte_output = merge_sliding_crops(matte_crops, crop_positions, original_width=W, overlap_size=args.overlap_size)
            nafnet_output = net_1(outputs)
            if args.base_loss.lower() == 'weightedchar':
                loss1 = matte_weighted_charbonnier_loss(nafnet_output, gt, matte_output)
            else:
                loss1 = nn.L1Loss()(nafnet_output, gt)
...
if __name__ == '__main__':
    ...
    if args.base_loss.lower() == 'weightedchar':
        base_loss = matte_weighted_charbonnier_loss
    else:
        base_loss = nn.L1Loss()
    ...
    while global_iter < args.iteration_target:
        for i, train_data in enumerate(train_loader, 0):
            ...
            matte_output = merge_sliding_crops(matte_crops, crop_positions, original_width=inputs.shape[-1], overlap_size=args.overlap_size)
            nafnet_output = net_1(outputs)
            if args.base_loss.lower() == 'weightedchar':
                loss1 = matte_weighted_charbonnier_loss(nafnet_output, labels, matte_output)
            else:
